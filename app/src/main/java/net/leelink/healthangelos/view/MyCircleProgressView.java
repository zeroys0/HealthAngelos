...
public class MyCircleProgressView extends View {
    ...
    int defaultWidth = 150; //默认宽高，单位sp
    ...
    public MyCircleProgressView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MyCircleProgressView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        mText = typedArray.getString(R.styleable.CircleProgressView_liys_progress_arc_text);
        mTextSize = typedArray.getDimensionPixelSize(R.styleable.CircleProgressView_liys_progress_arc_textSize, sp2px(defaultTextSize));

        mStartAngle = typedArray.getInt(R.styleable.CircleProgressView_liys_progress_arc_startAngle, defaultStartAngle);
        mDrawAngle = typedArray.getInt(R.styleable.CircleProgressView_liys_progress_arc_drawAngle, defaultDrawAngle);

        mTextColor = typedArray.getColor(R.styleable.CircleProgressView_liys_progress_arc_textColor, Color.parseColor(defaultTextColor));
        mInColor = typedArray.getColor(R.styleable.CircleProgressView_liys_progress_arc_inCircleColor, Color.parseColor(defaultInColor));
        mOutColor = typedArray.getColor(R.styleable.CircleProgressView_liys_progress_arc_outCircleColor, Color.parseColor(defaultOutColor));

        mInCircleSize = typedArray.getDimensionPixelSize(R.styleable.CircleProgressView_liys_progress_arc_inCircleSize, sp2px(defaultCircleSize));
        mOutCircleSize = typedArray.getDimensionPixelSize(R.styleable.CircleProgressView_liys_progress_arc_outCircleSize, sp2px(defaultCircleSize));
        typedArray.recycle();
...
//        int baseLine = mHeight / 3 + dy;
//        //3.绘制文字
//        canvas.drawText(mCurrentNum + "", mWidth / 2 - bounds.width() / 2, baseLine, mTextPaint);
//        mTextPaint2.setTextSize(sp2px(11));
//        DecimalFormat decimalFormat = new DecimalFormat("0");
//        canvas.drawText("推荐摄入量" +   decimalFormat.format(mMaxNum) + "千卡", mWidth / 3 - bounds.width() / 2, baseLine + mTextSize, mTextPaint2);
    }
